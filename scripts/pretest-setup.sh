...
log_dir="$HOME/logs"
log_file="$log_dir/libtools-testsetup.log"
...
function depcheck(){
    ...
    binary_depcheck awk grep aws libtools jq python3
...
function update_credentials(){
    ...
    local user="$1"
    #
    libtools --profile $PROFILE --user-name $user --operation up
...
std_message "${title}libtools${bodytext} Automated Testing Setup Check -- Start" "INFO"  $log_file
