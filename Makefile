...
upload-images:   ## Upload README images to Amazon S3
	bash $(CUR_DIR)/scripts/s3upload.py
...
clean:  clean-docs  ## Remove all build artifacts generated by make
	...
	rm -rf $(CUR_DIR)/__pycache__ || true
	rm -rf $(CUR_DIR)/$(PROJECT)/__pycache__ || true
	rm -rf $(CUR_DIR)/$(PROJECT)/colors/__pycache__ || true
	rm -rf $(CUR_DIR)/$(PROJECT)/common/__pycache__ || true
	rm -rf $(CUR_DIR)/$(PROJECT)/io/__pycache__ || true
	rm -rf $(CUR_DIR)/$(PROJECT)/js/__pycache__ || true
